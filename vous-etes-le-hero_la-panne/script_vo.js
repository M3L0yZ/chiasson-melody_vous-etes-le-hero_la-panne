...
/*
//NAVIGATION CONSOLE - ANCIENNE VERSION (SWITCH)
...
//NAVIGATION CONSOLE - NOUVELLE VERSION (BOUCLE)
/*
function goToChapter(chapter) {
...
let getTitre = document.querySelector("h1");
...
function goToChapter(chapter) {
    ...
    if (chapters[chapter].bouton == undefined) {
        ...
        addButton.className = "boutons-choix";
        getOptions.appendChild(addButton);
        ...
        chapters[chapter].bouton.forEach((bouton) => {
            let addButton = document.createElement("button");
            addButton.className = "boutons-choix";
            addButton.innerHTML = bouton.titre;
            
            addButton.addEventListener("click", () => {
                getOptions.querySelectorAll("*").forEach(bouton => bouton.remove());
    ...
    getTitre.textContent = chapters[chapter].titre;
    getDescription.textContent = chapters[chapter].description;
    getImage.src = chapters[chapter].image;
}
